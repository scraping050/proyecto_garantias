...
export default function ChatbotWidget() {
    ...
    // --- New External Reminder Logic ---
    useEffect(() => {
        // Define pages that need reminders
        const searchPages = ['/gestion-manual', '/busqueda-licitaciones', '/reportes'];
        const isSearchPage = searchPages.some(p => pathname?.includes(p));

        if (isSearchPage && !isOpen) {
            // 1. Initial trigger on mount/navigation
            setShowReminder(true);

            // 2. Interval every 60 seconds (1 minute)
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            // Use relative path for Next.js proxy to Node/Python backend
    ...
    return (
        typeof document !== 'undefined'
            ? createPortal(
                <div className="fixed bottom-0 right-0 z-[9999] sm:bottom-6 sm:right-6 flex flex-col items-end pointer-events-none font-sans">
                    {/* Chat Window */}
                    <div
                        ref={widgetRef}
                        className={cn(
                            "bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden transition-all duration-300 pointer-events-auto flex flex-col font-sans z-[9999]",
                    ...
                    {!isOpen && (
                        <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end pointer-events-auto group">

                            {/* Reminder Bubble */}
                            {showReminder && (
                                <div className="absolute bottom-24 right-0 mb-4 w-80 bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-2xl border-[3px] border-indigo-500/30 dark:border-indigo-500/50 animate-in slide-in-from-bottom-8 fade-in duration-500 z-50">
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setShowReminder(false); }}
                                        className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1 hover:bg-gray-100 dark:hover:bg-zinc-700 rounded-full transition-colors"
                                    >
                                        <X className="w-5 h-5" />
                                    </button>
                                    <div className="flex gap-4 items-start">
                                        <div className="flex-shrink-0 bg-gradient-to-br from-indigo-500 to-purple-600 p-3 rounded-xl shadow-lg h-fit">
                                            <Bot className="w-8 h-8 text-white" />
                                        </div>
                                        <div className="space-y-2 pr-4">
                                            <p className="text-base font-bold text-indigo-900 dark:text-indigo-100 leading-none">
                                                💡 Tip de AURA
                                            </p>
                                            <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed font-medium">
                                                ¿Nueva búsqueda? Recuerda usar el botón <b className="text-indigo-600 dark:text-indigo-400">Limpiar</b> 🔄 para obtener resultados precisos.
                                            </p>
                                        </div>
                                    </div>
                                    {/* Arrow */}
                                    <div className="absolute -bottom-3 right-8 w-6 h-6 bg-white dark:bg-zinc-800 border-b-[3px] border-r-[3px] border-indigo-500/30 dark:border-indigo-500/50 rotate-45 transform shadow-sm"></div>
                                </div>
                            )}
